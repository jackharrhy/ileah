defmodule Mix.Tasks.Commands.Create.Guild do
  ...
  If no guild_id is provided, uses ILEAH_TEST_GUILD_ID from config.
  ...
  def run([]) do
    case Application.get_env(:ileah, :test_guild_id) do
      nil ->
        Mix.shell().error("No guild_id provided and ILEAH_TEST_GUILD_ID not configured.")
  ...
  defp create_commands(guild_id) do
    ...
    case ILeah.Commands.create_guild_commands(guild_id) do
